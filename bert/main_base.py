...
def read_cail_examples(input_file, is_training):
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            # 经审理查明：2013年3月，武穴市卫生局下发《关于加强卫生监督协管工作的通知》，
            # 决定在各镇（处）卫生院设立卫生监督协管科，明确被告人黄某为刊江卫生院卫生监督协管工作第一责任人，
            # 被告人郭某为刊江卫生院卫生监督协管科科长。
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    output_weights1 = tf.get_variable(
        "cls/cail/output1_weights", [2, hidden_size],
        initializer=tf.truncated_normal_initializer(stddev=0.02))

    output_bias1 = tf.get_variable(
        "cls/cail/output1_bias", [2], initializer=tf.zeros_initializer())
    ...
    # start_logits.shape=[batch_size,seq_size]
    (start_logits, end_logits) = (unstacked_logits[0], unstacked_logits[1])

    # Yes-or-no type question classifier
    # pooled_hidden = model.get_pooled_output()
    # pooled_hidden_shape = modeling.get_shape_list(pooled_hidden, expected_rank=2)
    # p_hidden_size = pooled_hidden_shape[1]
    # output_weights2 = tf.get_variable(
    #     "cls/cail/output2_weights", [3, p_hidden_size],
    #     initializer=tf.truncated_normal_initializer(stddev=0.02))
    #
    # output_bias2 = tf.get_variable(
    #     "cls/cail/output2_bias", [3], initializer=tf.zeros_initializer())
    #
    # pooled_hidden_matrix = tf.reshape(pooled_hidden,
    #                                   [batch_size, p_hidden_size])
    # yon_logits = tf.matmul(pooled_hidden_matrix, output_weights2, transpose_b=True)
    # yon_logits = tf.nn.bias_add(yon_logits, output_bias2)
    #
    # yon_logits = tf.reshape(yon_logits, [batch_size, 3])
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            ...
            def compute_loss(logits, positions, d=seq_length):
                one_hot_positions = tf.one_hot(
                    positions, depth=d, dtype=tf.float32)
            ...
            # y_or_n = features["y_or_n"]
            start_loss = compute_loss(start_logits, start_positions)
            end_loss = compute_loss(end_logits, end_positions)
            # yon_loss = compute_loss(yon_logits, y_or_n, 3)
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    if is_training:
        ...
        name_to_features["y_or_n"] = tf.FixedLenFeature([], tf.int64)
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            # 对于不可回答的问题，记录start_logits[0]+end_logits[0]之和最小的feature的index
    ...
    for id in all_predictions.keys():
        answer = all_predictions[id]
    ...
    with tf.gfile.GFile(output_prediction_file, "w") as writer:
        ...
        writer.write(']')
        # writer.write(json.dumps(pred_dict, indent=4, ensure_ascii=False) + "\n")
